...
class AbstractArchiveIntegration extends AbstractTestNGSpringContextTests {
    private static Logger logger = LoggerFactory.getLogger(AbstractArchiveIntegration.class);
    ...
    private void testPhotoFlip(boolean initial) {
        ...
        Student student = createStudent(user, initial)
    ...
    private TwoPhaseActivity createActivity(AppUser user, boolean initial) {
        rwDAO.create(
    ...
    private void setObservableExpectations(boolean initial, String id, com.jtbdevelopment.e_eye_o.entities.Observable observable) {
    ...
    private Photo createPhoto(AppUser user, AppUserOwnedObject photoFor, String baseName, boolean initial) {
        rwDAO.create(
    ...
    private Student createStudent(AppUser user, boolean initial) {
        rwDAO.create(factory.newStudentBuilder(user).withFirstName("ArchivePhoto" + initial).withArchived(initial).build())
    }

    private AppUser createUser(String baseName) {
        rwDAO.create(factory.newAppUserBuilder().withFirstName(baseName).withEmailAddress(baseName + "@test.com").withPassword("X").build())
    }
