 ...
 */
abstract class AbstactIdObjectBaseTest {
    ...
    protected testStringFieldExpectingErrorForNullOnly(
            def String field, def String defaultValue, def String expectedError) {
        testStringFieldWithValidationError(field, defaultValue, expectedError, true)
    }

    protected testStringFieldExpectingErrorForNullOrBlank(
            def String field, def String defaultValue, def String expectedError) {
        testStringFieldWithValidationError(field, defaultValue, expectedError, false)
    }

    private testStringFieldWithValidationError(
            def String field, def String defaultValue, def String expectedError, def Boolean blankOK) {
    ...
    protected testNonStringFieldWithNullValidationError(
            def String field, def defaultValue, def testValue, def String expectedError) {
        assert objectUnderTest."$field" == defaultValue;
        objectUnderTest."$field" = NULL;
        testValidation([expectedError], [])
    ...
    protected testSetField(
            def String field, def String upperSingle, def value1, def value2, def String containsNullError) {
