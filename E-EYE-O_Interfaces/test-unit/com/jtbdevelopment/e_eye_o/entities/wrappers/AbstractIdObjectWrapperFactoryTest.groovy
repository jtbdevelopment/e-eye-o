...
abstract class AbstractIdObjectWrapperFactoryTest {
    IdObjectWrapperFactory wrapperFactory;
    ...
    abstract IdObjectWrapperFactory makeFactory()

    @BeforeMethod
    public void setUp() {
        wrapperFactory = makeFactory()
    }
    ...
    public void testCollectionBlowsUpOnUnsupportedCollection() {
        ...
        collections.each { def c ->
            boolean exception = false;
            try {
                wrapperFactory.wrap(IdObjectWrapperFactory.WrapperKind.DAO, c);
            } catch (RuntimeException e) {
                exception = true;
                assert "Don't know how to construct collection of " + c.getClass().getSimpleName() == e.message
            }
            assert exception, "Should have had exception"
        }
    }
